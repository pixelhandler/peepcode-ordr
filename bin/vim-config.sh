...
# use like `vagrant ssh` then `/vagrant/bin/vim_config.sh`
...
if [ -d $VIMDIR ]; then echo 'vim config already setup in '$VIMDIR
  ...
  # install vim
  sudo apt-get install gpm ctags vim vim-doc
  # you may also want vim-scripts
  # vim setup with nice bundles using pathogen all setup to clone and use
